...
nav = '<a href="#gmarket">📊 大盘</a><a href="#gtrend">📈 趋势</a>'
for c in order:
...
# 大盘日报
market_data = data.get('market_summary', {})
market_html_sec = ''
if market_data:
    indices = market_data.get('indices', [])
    mi = ''
    for idx in indices:
        n = escape(idx.get('name',''))
        p = escape(str(idx.get('price','')))
        ch = escape(str(idx.get('change','')))
        cls = 'up' if (isinstance(ch,str) and ch.startswith('+')) else 'down' if (isinstance(ch,str) and ch.startswith('-')) else ''
        mi += '<div class="ri"><span class="rn">'+n+'</span><span class="sv">'+p+'</span><span class="rv '+cls+'">'+ch+'</span></div>'
    # 板块涨跌
    sector_up = market_data.get('sectors_up', [])[:3]
    sector_down = market_data.get('sectors_down', [])[:3]
    sec_html = ''
    if sector_up:
        sec_html += '<div style="display:flex;flex-wrap:wrap;gap:2px;margin-top:4px">'
        for s in sector_up:
            sec_html += '<span style="font-size:8px;color:#22c55e;background:rgba(34,197,94,0.06);padding:1px 4px;border-radius:3px">'+escape(s.get('name',''))+' '+escape(str(s.get('pct','')))+'%</span>'
        sec_html += '</div>'
    if sector_down:
        sec_html += '<div style="display:flex;flex-wrap:wrap;gap:2px;margin-top:2px;margin-bottom:2px">'
        for s in sector_down:
            sec_html += '<span style="font-size:8px;color:#ef4444;background:rgba(239,68,68,0.06);padding:1px 4px;border-radius:3px">'+escape(s.get('name',''))+' '+escape(str(s.get('pct','')))+'%</span>'
        sec_html += '</div>'
    zt_count = market_data.get('zt_count', 0)
    dt_count = market_data.get('dt_count', 0)
    summary = '<div style="font-size:9px;color:#4a5a6d;margin-top:3px">涨停'+str(zt_count)+' 跌停'+str(dt_count)+' | '+escape(market_data.get('summary',''))+'</div>' if market_data.get('summary') else ''
    market_html_sec = '<div class="se" id="gmarket"><div class="sh"><span class="st">📊 A股大盘日报</span><span class="sc">实时</span></div><div class="rg">'+mi+'</div>'+sec_html+summary+'</div>'

# 周趋势
weekly_data = market_data.get('weekly', None)
weekly_html = ''
if weekly_data:
    wk = weekly_data
    direction_class = 'up' if wk.get('range','').startswith('+') else 'down'
    wi = '<div class="ri"><span class="rn">'+escape(wk.get('label',''))+'</span><span class="sv">'+escape(wk.get('close',''))+'</span><span class="rv '+direction_class+'">'+escape(wk.get('range',''))+'</span></div>'
    wi += '<div style="font-size:9px;color:#4a5a6d;margin-top:3px">'
    wi += '开:'+escape(wk.get('open',''))+' 高:'+escape(wk.get('high',''))+' 低:'+escape(wk.get('low',''))+' 振幅:'+escape(wk.get('swing',''))
    if wk.get('max_up_days',0) >= 3:
        wi += ' 连涨'+str(wk['max_up_days'])+'天'
    if wk.get('max_down_days',0) >= 3:
        wi += ' 连跌'+str(wk['max_down_days'])+'天'
    wi += '</div>'
    wi += '<div style="font-size:10px;color:#818cf8;margin-top:2px">'+escape(wk.get('summary',''))+'</div>'
    weekly_html = '<div class="se"><div class="sh"><span class="st">📈 一周走势</span><span class="sc">'+escape(wk.get('start_date',''))+' ~ '+escape(wk.get('end_date',''))+'</span></div>'+wi+'</div>'

# 月趋势
monthly_data = market_data.get('monthly', None)
monthly_html = ''
if monthly_data:
    mk = monthly_data
    direction_class = 'up' if mk.get('range','').startswith('+') else 'down'
    mi2 = '<div class="ri"><span class="rn">'+escape(mk.get('label',''))+'</span><span class="sv">'+escape(mk.get('close',''))+'</span><span class="rv '+direction_class+'">'+escape(mk.get('range',''))+'</span></div>'
    mi2 += '<div style="font-size:9px;color:#4a5a6d;margin-top:3px">'
    mi2 += '开:'+escape(mk.get('open',''))+' 高:'+escape(mk.get('high',''))+' 低:'+escape(mk.get('low',''))+' 振幅:'+escape(mk.get('swing',''))
    if mk.get('max_up_days',0) >= 4:
        mi2 += ' 连涨'+str(mk['max_up_days'])+'天'
    if mk.get('max_down_days',0) >= 3:
        mi2 += ' 连跌'+str(mk['max_down_days'])+'天'
    mi2 += '</div>'
    mi2 += '<div style="font-size:10px;color:#818cf8;margin-top:2px">'+escape(mk.get('summary',''))+'</div>'
    monthly_html = '<div class="se"><div class="sh"><span class="st">📉 月度趋势</span><span class="sc">'+escape(mk.get('start_date',''))+' ~ '+escape(mk.get('end_date',''))+'</span></div>'+mi2+'</div>'
...
for c in order:
    ...
    for item in all_items:
        t10 = item.get('t','')[:10]
        cnt = sum(1 for x in all_items if t10 in x.get('t','') and x.get('src') != item.get('src'))
        multi_map[id(item)] = cnt
...
body += market_html_sec + weekly_html + monthly_html + news_html
body += market_html + metal_html + crypto_html + vol_html + q_html + gh_html + rank_html + shop_html + zh_html
body += hl_html + hw_html
# ===== 来源折叠（默认显示5个，其余折叠）=====
src_display = src_html.count('</a>')  # 粗略计数（实际src_html里是纯文本，不在">内）
src_parts = src_html.split(' · ')
if len(src_parts) > 5:
    src_show = ' · '.join(src_parts[:5])
    src_hidden = ' · '.join(src_parts[5:])
    src_html = '<span class="srcv">'+src_show+'</span><span class="srch" style="display:none"> · '+src_hidden+'</span> <span class="srct" onclick="this.previousSibling.style.display=this.previousSibling.style.display==\'none\'?\'inline\':\'none\';this.textContent=this.textContent==\'展开全部\'?\'收起\':\'展开全部\'" style="color:#818cf8;cursor:pointer;font-size:9px">展开全部</span>'
else:
