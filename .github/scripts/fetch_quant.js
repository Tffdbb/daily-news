...
async function fetchStocks(page, pz) {
  // fid=f62 按成交额降序取，覆盖最活跃的股�?  const url = `https://push2.eastmoney.com/api/qt/clist/get?pn=${page}&pz=${pz}&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f62&fs=m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23&fields=${FIELDS}`;
  try {
...
function pick(stocks) {
  ...
  for (const s of stocks) {
    const name = s.f14 || '';
    const code = s.f12 || '';
    // 过滤ST/退�?    if (name.includes('ST') || name.includes('退') || name.includes('N')) continue;
    const chg = s.f3 || 0;
    // 涨幅>1%�?18%（非涨停，但要有趋势�?    if (chg < 1.0 || chg > 18.0) continue;
    ...
    // 综合评分
    // f38字段是累计值（原始），实际换手�?在东方财富用f38=成交�?流通股�?    // 实际f38单位已是%，但值是整数�?4.31 表示4.31%
    // 如果turnover�?100说明是原始累计值，/10
    ...
    score += Math.min(amt / 5, 1.0) * 15;        // 成交�?    score += Math.min(circCap / 30, 1.0) * 15;   // 市�?    if (pe && pe > 10 && pe < 50) score += 15;         // PE适中

    // 换手率显示简化（f38原始值无法直接转%�?    const turnDisplay = turnover > 1e6 ? (turnover/1e8).toFixed(1) + '%' : turnover.toFixed(1) + '%';
...
async function main() {
  let all = [];
  // 按成交额（f62）取�?00只，比按涨幅更合�?  for (const page of [1, 2, 3, 4, 5]) {
  ...
  console.log(`扫描${all.length}�? 选出${picks.length}只`);
  for (const p of picks) {
    console.log(`  ${p.code} ${p.name} 涨幅${p.chg >= 0 ? '+' : ''}${p.chg}% 量比${p.volRatio} 换手${p.turnover}% PE${p.pe} 成交${p.amt}�?评分${p.score}`);
  }
}
